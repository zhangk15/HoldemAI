...
patten_head = re.compile(r'(([\w|-]+)/\s+(.+)\s+/(\2)\s*|game-over\s+)', __flag)

patten_seat = re.compile(r'\s*(\d+)\s+(\d+)\s+(\d+)\s*', __flag)
patten_blind = re.compile(r'\s*(\d+):\s*(\d+)\s*', __flag)
...
patten_shutdown_rank = re.compile(r'(\d):\s*(\d+)\s+(SPADES|HEARTS|CLUBS|DIAMONDS)\s+([2-9]|10|J|Q|K|A)\s+(SPADES|HEARTS|CLUBS|DIAMONDS)\s+([2-9]|10|J|Q|K|A)\s+(HIGH_CARD|ONE_PAIR|TWO_PAIR|THREE_OF_A_KIND|STRAIGHT|FLUSH|FULL_HOUSE|FOUR_OF_A_KIND|STRAIGHT_FLUSH)\s*', __flag)
